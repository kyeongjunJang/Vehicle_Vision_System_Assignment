...
[m1_concat, m2_concat] = get_correspondence_points(undistortImage1, undistortImage2, 1);
...
for i = 2:28
    ...
    [m1, m2] = get_correspondence_points(undistortImage1,undistortImage2, 0);
...
drawnow; figure; imshow(Image1); title('Raw Image');

drawnow; figure; imshow(undistortImage1); title('Image Undistortion'); 
...
drawnow; figure; imshow([I1Rect, I2Rect]); title('Image Rectification');

%% Get Disparity Map
disparityImage = get_disparity_map(I1Rect, I2Rect);

drawnow; figure; imshow(disparityImage); title('Disparity Map');clim([-100 100]);
% colormap gray;
% colorbar ;

disparityImage_with_DP = get_disparity_map_with_dynamic_programming(I1Rect, I2Rect);

drawnow; figure; imshow(disparityImage_with_DP); title('Disparity Map with Dynamic Programming');clim([-100 100]);
% colormap gray;
% colorbar ;
